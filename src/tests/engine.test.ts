...
describe('Battle Engine', () => {
  ...
  describe('Rout Handling', () => {
    it('should trigger rout when morale <= 20', () => {
      ...
      // Apply terrain morale modifiers first
      const { attacker: initAttacker, defender: initDefender } = engine.getArmies();
  ...
  describe('Decision Phase', () => {
    ...
    it('should give defender victory on exact decision power equality', () => {
      ...
      battle = createBattle('war1', 'test_zupa', 'field', equalAttacker.id, equalDefender.id, 0, 'equal-decision-seed');
      engine = new BattleEngine(battle, equalAttacker, equalDefender);
      ...
      if (!engine.isFinished()) {
        ...
        // Defender should win on equality
        expect(engine.getWinnerArmyId()).toBe(equalDefender.id);
      }
    });
  });
  ...
  describe('RNG State', () => {
    ...
    it('should save and restore RNG state correctly', () => {
      ...
      // Execute same phase - should produce same results
      ...
      // Note: This might not be exactly equal due to floating point, but should be close
      expect(phaseLog1.attackerLosses).toBeCloseTo(phaseLog2.attackerLosses, 1);
      expect(phaseLog1.defenderLosses).toBeCloseTo(phaseLog2.defenderLosses, 1);
    });
  });
  ...
  describe('AI Action Selection', () => {
    ...
    it('should select retreat when morale < 35 and size < 50%', () => {
      const weakArmy: Army = {
        id: 'weak_army',
        factionId: 'test',
        size: 200, // Less than 50% of original (which would be 1000)
      ...
      battle = createBattle('war1', 'test_zupa', 'field', weakArmy.id, defender.id, 0, 'retreat-test-seed');
      engine = new BattleEngine(battle, weakArmy, defender);
      
      // Set original size to 1000 so current size (200) is < 50%
      // We need to access private field, so we'll test through the public API
      const action = engine.selectAIAction(true);
