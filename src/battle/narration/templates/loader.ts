...
// JSON module imports lose their literal string types (slot/conditions come back
// as plain `string`), so we assert the shape here once rather than annotating every
// template file with `as const`.
...
export function validateTemplateCounts(): {
  ...
  // Záväzné minimá zo spec §8 (nie odvodené z aktuálneho počtu súborov)
  const required: Record<string, number> = {
    openers: 18, // 6 per phase * 3 phases
    actions: 36, // 4 per (attackerAction x outcome) bucket * 9 buckets
    terrain: 15, // 3 per terrain * 5 terrains
    outcomes: 24, // 4 per (outcome x intensity) bucket * 6 buckets
