...
import type { Army, BattlePhaseType, BattleAction, Terrain, UnitType } from './types';
...
// 6.1 Efektívna sila
// Poznámka: ES je deterministická podľa vzorca zo spec (§6) - rng parameter je
// zámerne nepoužitý v tejto funkcii, náhodnosť sa aplikuje až na úrovni fázy (rng.range).
export function calculateEffectiveStrength(
  ...
  for (const [unitType, ratio] of Object.entries(army.composition)) {
    compositionFactor += ratio * UNIT_TERRAIN_MULTIPLIERS[unitType as UnitType][terrain];
  }
...
// Pri presnej rovnosti decisionPower vyhráva obranca (§6.7). Vytiahnuté ako čistá
// funkcia, aby sa dala jednotkovo otestovať bez závislosti na RNG.
export function determineDecisionWinner(
  decisionPowerAttacker: number,
  decisionPowerDefender: number
): 'attacker' | 'defender' {
  return decisionPowerAttacker > decisionPowerDefender ? 'attacker' : 'defender';
}
...
export function evaluateDecisionPhase(
  ...
  const winner = determineDecisionWinner(decisionPowerAttacker, decisionPowerDefender);
...
export function applyTerrainMoraleModifiers(
  ...
  // Iba spodná hranica 0 - terénový bonus môže dočasne vytiahnuť morálku nad 100
  // (klampuje sa až pri ďalších zmenách morálky počas fáz, viď engine.ts).
