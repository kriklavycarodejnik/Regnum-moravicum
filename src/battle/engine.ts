...
export function shouldAIRetreat(army: Army, originalSize: number): boolean {
...
export class BattleEngine {
  ...
  constructor(battle: Battle, attacker: Army, defender: Army, originalSizes?: { attacker: number; defender: number }) {
    this.battle = { ...battle };
    this.attacker = cloneArmy(attacker);
    this.defender = cloneArmy(defender);
    this.originalAttackerSize = originalSizes?.attacker ?? attacker.size;
    this.originalDefenderSize = originalSizes?.defender ?? defender.size;

    // Initialize RNG
    this.rng = new BattleRNG(battle.seed, (battle.rngState as RNGState | null) || undefined);

    // Terrain morale modifiers apply once, at the true start of a battle. If we're
    // reconstructing an engine mid-battle (save/load), battle.phaseLogs already
    // reflects earlier phases and attacker/defender are the current, already-adjusted
    // army snapshots - reapplying the terrain bonus here would double-count it.
    if (battle.phaseLogs.length === 0 && battle.currentPhase !== 'finished') {
  ...
  setRNGState(state: object): void {
    this.rng = new BattleRNG(this.battle.seed, state as RNGState);
  }
  ...
  executeFullBattle(
    ...
    // Phase 1: Attack
    if (this.battle.currentPhase === 'attack') {
    ...
    if (this.battle.currentPhase === 'counterattack' || (this.battle.currentPhase as BattlePhaseType) === 'attack') {
